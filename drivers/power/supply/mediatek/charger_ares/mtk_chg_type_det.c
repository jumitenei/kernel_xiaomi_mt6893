...
void set_fake_plug_out(bool value)
{
	struct power_supply	*usb_psy;
	struct mt_charger *mtk_chg = NULL;
	usb_psy = power_supply_get_by_name("usb");
	mtk_chg = power_supply_get_drvdata(usb_psy);
	if (value) {
		mtk_chg->fake_plug_in_out = mtk_chg->fake_plug_in_out + 2;
	}
}
EXPORT_SYMBOL(set_fake_plug_out);
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if ((mtk_chg->chg_type == STANDARD_HOST) ||
			(mtk_chg->chg_type == CHARGING_HOST))
			val->intval = 0;
		if (cti->tcpc_kpoc && mtk_chg->fake_plug_in_out < 2) {
			val->intval = 1;
			pr_err("%s [mi_chg] : %d-%d-%d\n", __func__, cti->tcpc_kpoc, mtk_chg->fake_plug_in_out, val->intval);
		}
		break;
	default:
		return -EINVAL;
